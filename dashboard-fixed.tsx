...
import { useDashboardData } from "./hooks/useDashboardData-debug"

export default function VRDashboardFixed() {
  ...
  // Crear usuarios únicos para evitar keys duplicadas
  const usuariosUnicos = data.usuarios.reduce(
    (acc, usuario) => {
      const key = `${usuario.id}-${usuario.evento_id}`
      if (!acc.find((u) => `${u.id}-${u.evento_id}` === key)) {
        acc.push(usuario)
      }
      return acc
    },
    [] as typeof data.usuarios,
  )
            ...
            <CardContent>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data.edades} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    />
                    <Bar dataKey="cantidad" fill="#10b981" radius={[4, 4, 0, 0]} name="Cantidad de usuarios" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </CardContent>
        ...
        <Card className="bg-white shadow-sm mb-8">
          ...
          <CardContent>
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.simuladores} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  <XAxis
                    dataKey="nombre"
                    tick={{ fontSize: 11 }}
                  ...
                  />
                  <Bar dataKey="usos" fill="#f59e0b" radius={[4, 4, 0, 0]} name="Número de usos" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="bg-white shadow-sm mb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <CardTitle className="text-lg font-semibold text-slate-900">
                  Usuarios Registrados ({usuariosUnicos.length})
                </CardTitle>
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {usuariosUnicos.length > 0 ? (
                    usuariosUnicos.slice(0, 20).map((usuario, index) => (
                      <TableRow key={`${usuario.id}-${usuario.evento_id}-${index}`} className="hover:bg-slate-50">
              ...
              {usuariosUnicos.length > 20 && (
                <p className="text-xs text-slate-500 mt-2 text-center">
                  Mostrando los primeros 20 de {usuariosUnicos.length} usuarios
                </p>
              )}
            </div>
          </CardContent>
