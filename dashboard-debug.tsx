...
export default function VRDashboardDebug() {
          ...
          <AlertDescription className="mt-2">
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-600">
                Revisa la consola del navegador (F12) para más detalles sobre el error.
              </p>
            </div>
          </AlertDescription>
        </Alert>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Alert>
            ...
            <AlertDescription>
              <div className="space-y-1 text-sm">
                ...
                <p>• Usuarios: {data.usuarios.length}</p>
                <p>• Datos de género: {data.genero.length}</p>
                <p>• Datos de edades: {data.edades.length}</p>
                <p>• Simuladores: {data.simuladores.length}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="bg-white shadow-sm">
            <CardHeader>
              ...
              <CardDescription className="text-slate-600">
                Usuarios registrados por rango de edad ({data.edades.length} rangos)
              </CardDescription>
            </CardHeader>
            <CardContent>
              {data.edades.length > 0 ? (
                ...
                <div className="h-[300px] flex items-center justify-center text-slate-500">
                  <div className="text-center">
                    ...
                    <p className="text-xs mt-1">Usuarios encontrados: {data.usuarios.length}</p>
                  </div>
                </div>
              )}
            </CardContent>
          ...
          <Card className="bg-white shadow-sm">
            <CardHeader>
              ...
              <CardDescription className="text-slate-600">
                Proporción de usuarios por género ({data.genero.length} categorías)
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card className="bg-white shadow-sm mb-8">
          <CardHeader>
            ...
            <CardDescription className="text-slate-600">
              Ranking de simuladores por número de usos ({data.simuladores.length} simuladores)
            </CardDescription>
          </CardHeader>
          <CardContent>
            {data.simuladores.length > 0 ? (
              ...
              <div className="h-[400px] flex items-center justify-center text-slate-500">
                <div className="text-center">
                  ...
                  <p className="text-xs mt-1">Revisa la tabla event_packages en tu base de datos</p>
                </div>
              </div>
            )}
          </CardContent>
        ...
        <Card className="bg-white shadow-sm mb-6">
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {data.usuarios.length > 0 ? (
                    data.usuarios.slice(0, 10).map((usuario) => (
              ...
              {data.usuarios.length > 10 && (
                <p className="text-xs text-slate-500 mt-2 text-center">
                  Mostrando los primeros 10 de {data.usuarios.length} usuarios
                </p>
              )}
            </div>
          </CardContent>
